...
{
	auto owningManager = GetOwningUIManager();
	
	if (owningManager)
		return nullptr;
	
	auto gameInstance = owningManager->GetGameInstance();
...
{
	UE_LOG(LogFrogsmithUI, Log, TEXT("[%s] is adding player [%s]'s root layout [%s] to the viewport"), *GetName(), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));
...
void UGameUIPolicy::RemoveLayoutFromViewport(UCommonLocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
	...
	UE_LOG(LogFrogsmithUI, Log, TEXT("[%s] is removing player [%s]'s root layout [%s] from the viewport"), *GetName(), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));

	Layout->RemoveFromParent();
	if (LayoutSlateWidget.IsValid())
	{
		UE_LOG(LogFrogsmithUI, Log, TEXT("Player [%s]'s root layout [%s] has been removed from the viewport, but other references to its underlying Slate widget still exist. Noting in case we leak it."), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));
	}
...
void UGameUIPolicy::CreateLayoutWidget(UCommonLocalPlayer* LocalPlayer)
{
	...
	const auto LayoutWidgetClass = GetLayoutWidgetClass(LocalPlayer);
