...
class FROGSMITHUI_API UPrimaryGameLayout : public UCommonUserWidget
{
	GENERATED_BODY()
...
public:
	bool HasWidget(TSubclassOf<UCommonActivatableWidget> WidgetClass, FGameplayTag Layer = FGameplayTag()) const;
	...
	TSharedPtr<FStreamableHandle> PushWidgetToLayerStackAsync(FGameplayTag LayerName, bool bSuspendInputUntilComplete, TSoftClassPtr<UCommonActivatableWidget> ActivatableWidgetClass, TFunction<void(EAsyncWidgetLayerState, ActivatableWidgetT*)> StateFunc)
	{
		...
		TSharedPtr<FStreamableHandle> StreamingHandle = StreamableManager.RequestAsyncLoad(ActivatableWidgetClass.ToSoftObjectPath(), FStreamableDelegate::CreateWeakLambda(this,
			[this, LayerName, ActivatableWidgetClass, StateFunc, SuspendInputToken]()
			{
				...
				UClass* LoadedClass = ActivatableWidgetClass.Get();
				if (!LoadedClass)
				{
					UE_LOG(LogTemp, Error, TEXT("PushWidgetToLayerStackAsync: Failed to load widget class '%s' - asset may be missing from the build"), *ActivatableWidgetClass.ToString());
					StateFunc(EAsyncWidgetLayerState::Canceled, nullptr);
					return;
				}

				ActivatableWidgetT* Widget = PushWidgetToLayerStack<ActivatableWidgetT>(LayerName, LoadedClass, [StateFunc](ActivatableWidgetT& WidgetToInit) {
